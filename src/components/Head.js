...
const Head = () => {
    ...
    return (
        <>
            ...
            <nav className={'flex font-comf group bg-slate-200 text-slate-200 transition-all ease-in-out duration-300 w-full sticky top-[2rem] pt-[0.25rem] ' + 
                ...
                <div className='bg-inherit z-10 justify-self-start'>
                    <img src='https://res.cloudinary.com/meme-topia/image/upload/v1723784096/image-removebg-preview_ciglfw.png' alt='Vec Logo'
                        className='group-[.hide]:w-[2.5rem] group-[.hide]:h-[2.5rem] z-10 bg--white duration-300 ease-in-out transition-all w-[5rem] h-[5rem]'></img>
                </div>    
                <div className='ml-2 mr-[25vw] group-[.hide]:-translate-x-[20vw] duration-500 ease-out transition-all'>
                    <p className='mt-4 font-rome text-2xl text-amber-800'>VELAMMAL</p>
                    <p className='font-rome text-black text-xs'>ENGINEERING COLLEGE</p>
                </div>
                <div className='flex-1 mr-8'>
                    <div className='flex items-stretch group-[.hide]:text-lg transition-all text-2xl justify-end h-full gap-4'>
                        {navs.map((nvt) => (
                            <p className='text-slate-950 group-[.hide]:pt-1.5 pt-5 relative text-center self-center my-auto inline border w-fit px-4 hover:after:scale-y-100 
                                bg-slate-300 h-full flex-end after:content-[""] after:absolute after:w-full after:h-full after:origin-bottom after:scale-y-0 after:bg-amber-500
                ...
                <Sidebar Sz={((hdr === "h-20") ? "fll": "tny p-0 mt-[1.5rem]")} /></div>        
            </nav>
        </> 
    )
}
